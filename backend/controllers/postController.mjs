...
const createPost = async (req, res) => {
    try {
        ...
        if (!postedBy || !text) {
            return res.status(400).json({ error: "postedBy and text fills required" })
        }
        ...
        if (text.length > maxLength) {
            return res.status(400).json({ error: `Text must be less than ${maxLength} charachter` })
        }
...
const likeUnlikePost = async (req, res)=>{
    try {
        ...
        res.status(500).json({ message: err.message })
        console.log("Error in likeUnlike post:", err.message)
    }
}
