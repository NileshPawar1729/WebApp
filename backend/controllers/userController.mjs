...
import generateToken from '../utils/helpers/generateToken.mjs';
...
const updateUser = async (req, res) => {
    ...
    try {
        ...
        if (req.params.id !== userId.toString()) return res.status(400).json({ error: "You cannot update other user profiles" });
...
const getUserProfile = async (req, res) => {
    const { query } = req.params;
     //we will fetch user with username or userid
    try {
         let user;
